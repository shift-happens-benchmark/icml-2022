...
def build_map_dict():
    map_dict = {}
    with open("examples/imagenet_d_helper/imagenet_classes.csv", "r") as r:
        for line in r:
            splits = line.split("\001")
            map_dict[int(splits[0])] = splits[1]
    return map_dict
...
def create_symlinks_and_get_imagenet_visda_mapping(visda_location, symlinks_location, map_dict):
    ...
    for folder in matching_names.keys():
        target_folder_class = os.path.join(symlinks_location, ambiguous_matching[folder])
    ...
    matching_names, matching_labels = get_imagenet_visda_mapping(symlinks_location, map_dict)
